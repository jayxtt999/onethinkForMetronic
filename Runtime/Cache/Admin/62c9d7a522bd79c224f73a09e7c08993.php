				...
				<div class="portlet-body">
					<div class="table-toolbar">
						<div class="row">
							<div class="col-md-6 cf">
								<div class="fl">
									<button class="btn green " id="model_add" url="<?php echo U('Model/add');?>">新 增</button>
									<button class="btn green ajax-post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>1));?>">启 用</button>
									<button class="btn green ajax-post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>0));?>">禁 用</button>
									<button class="btn green " id="model_generate" url="<?php echo U('Model/generate');?>">生 成</button>
								</div>
							</div>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="dataTables-model"><!-- 必须设定表格的id == -->
						...
						<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
								<td><input class="checkboxes ids" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>" /></td>
